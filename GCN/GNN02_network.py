...
from torch_geometric.nn import (
    GATConv,
    TransformerConv,          # ← 新增：用于有向阶段，支持 edge_attr
...
class NetA_NodeOnly(nn.Module):
    ...
    节点图两阶段（无向→有向），无池化；支持门控加权融合：
      - Stage-1 无向：GATConv ×2 -> H_und
      - Stage-2 有向：TransformerConv ×2 (使用 edge_attr) -> H_dir
    ...
    def __init__(self, lambda_consistency: float = 0.0, hidden_dim: int = 256,
        ...
        self.edge_dim = edge_dim  # 边特征维度，与你的 edge_attr 列数一致（WIN,PRE,TMP,PRS,SHU,flow -> 6）
        ...
        # 使用 TransformerConv，使边特征参与消息/注意力计算
