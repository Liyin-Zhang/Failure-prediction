"""
Brief (English)
Parallel undirected and directed design. Discrete node features are embedded, then two
TransformerConv layers encode the undirected graph to get H_und and another two layers
encode the directed graph to get H_dir. Both branches are pooled by GAP and GMP. A gating
MLP fuses them at feature level or logit level to produce graph level binary outputs.
An optional consistency regularizer is supported.
"""
...
class NetA_NodeOnly(nn.Module):
    """
    Parallel undirected and directed node level encoder with gated fusion.

    Expected data fields:
      - x: LongTensor [N, feature_num] discrete indices expanded via embedding
      - edge_index_dir: LongTensor [2, E_dir] directed edges
      - edge_attr: FloatTensor [E_dir, edge_dim] directed edge attributes
      - optional edge_index_und and edge_attr_und for undirected graph
      - batch: LongTensor [N] node to graph mapping

    Notes:
      The symbols max_node_number, embed_dim, and feature_num must be defined at
      module scope before constructing the model for example via an args module.
      Set lambda_consistency greater than zero to enable the consistency loss.
    """
    def __init__(
        self,
        ...
        self.item_embedding = nn.Embedding(
            num_embeddings=max_node_number,
    ...
    def _get_undirected_with_attrs(self, data):
        """
        Return undirected edges and attributes.
        Prefer provided edge_index_und and edge_attr_und.
        Otherwise derive them from directed edges via to_undirected with mean reduce.
        """
        if (
    ...
    def forward(self, data, return_reg: bool = False):
        ...
        # Undirected graph and attributes
        ...
        if (not return_reg) or self.lambda_consistency <= 0.0:
            return logit  # return logits

        # Consistency regularizer at node level
