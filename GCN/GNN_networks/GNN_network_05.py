...
This model uses a cascaded Undirected→Directed design: two TransformerConv layers encode
the undirected graph to produce H_und, then H_und is fed into two TransformerConv layers
on the directed graph to produce H_dir. Both representations are pooled via GAP/GMP and
fused by a gating mechanism (feature-level or logit-level) for binary prediction.
An optional node-level consistency regularizer is supported.
"""
...
class NetA_NodeOnly(nn.Module):
    """
    Two-stage (Undirected → Directed) node-level encoder without internal pooling.
    Supports gated fusion at feature level or logit level.
    ...
    Note:
      The symbols `max_node_number`, `embed_dim`, and `feature_num` are expected to be
      defined at module scope (e.g., injected from a config/args module before constructing
      the model). Set `lambda_consistency > 0` to enable the consistency regularizer.
    """
    def __init__(
        self,
        lambda_consistency: float = 0.0,
        hidden_dim: int = 256,
        fuse_mode: str = 'feature',
        dropout_p: float = 0.5,
        edge_dim: int = 6,
    ):
        super().__init__()
        ...
        # Stage-1: Undirected (uses undirected edge attributes)
        ...
        # Stage-2: Directed (uses directed edge attributes), cascaded from H_und
