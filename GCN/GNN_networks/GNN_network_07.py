...
Parallel Undirected and Directed (GATv2) design: starting from the same embedded node
features, two GATv2Conv layers encode the undirected graph to get H_und and another two
GATv2Conv layers encode the directed graph to get H_dir. Both branches are pooled with
GAP and GMP, then fused by a gating MLP (feature level or logit level) to produce a
binary probability. An optional consistency regularizer is supported.
"""
...
class NetA_NodeOnly(nn.Module):
    """
    Parallel (Undirected and Directed) node-level encoder with GATv2 and gated fusion.
    ...
    def __init__(
        ...
        self.und_conv1 = GATv2Conv(d_in, d_h, heads=1, dropout=0.0, edge_dim=edge_dim)
        self.und_conv2 = GATv2Conv(d_h,  d_h, heads=1, dropout=0.0, edge_dim=edge_dim)

        # Directed branch (uses directed edge attributes)
        self.dir_conv1 = GATv2Conv(d_in, d_h, heads=1, dropout=0.0, edge_dim=edge_dim)
        self.dir_conv2 = GATv2Conv(d_h,  d_h, heads=1, dropout=0.0, edge_dim=edge_dim)
    ...
    def _get_undirected_with_attrs(self, data):
        ...
        edges and attributes via to_undirected(..., reduce='mean').
        """
        if (
