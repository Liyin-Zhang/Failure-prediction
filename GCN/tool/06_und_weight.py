...
# ========= 自行设置 =========
anzhao     = "GNN_data/evaluate/evaluate_0415_3_filtered_3.csv"
...
FEEDER_TARGET = "10kV石滩F8碧江线03447"#"东方F3棠吓线03364"
TOPK_EDGES = 30
TOPK_NODES = 20
...
class WrappedModel(torch.nn.Module):
    """
    适配你修改后的 GNNExplainer：forward(x, edge_index, **kwargs)
    - kwargs 中应包含 edge_attr, batch
    - 将 explainer 提供的 edge_index/edge_attr 作为“无向阶段”的输入，保证与 mask 对齐
    """
    def __init__(self, base_model: NetA_NodeOnly, proto_data: Data):
        super().__init__()
        self.base = base_model
        # 固定使用原样本中的离散索引与 batch 作为模板兜底
    ...
    def forward(self, x, edge_index, **kwargs):
        ...
        g = Data()
        # 你的模型需要离散索引，这里忽略 explainer 的 x，直接用原始离散索引
        g.x = self.proto_x

        # ★ 关键：把 explainer 的边当作“无向阶段”的边（与 edge_mask 一致）
        g.edge_index_und = edge_index
        g.edge_attr_und  = edge_attr

        # 若模型里还会读取 dir 字段，一并映射，避免 None 造成分支进入 to_undirected：
        g.edge_index_dir = edge_index
        g.edge_attr      = edge_attr

        g.batch = batch if batch is not None else self.proto_batch
