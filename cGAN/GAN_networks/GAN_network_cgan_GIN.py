...
class Generator(nn.Module):
    def __init__(self, feature_dim, noise_dim, hidden_dim, dropout_prob=0.3):
        ...
        self.conv1 = GINConv(nn.Sequential(
            nn.Linear(feature_dim + noise_dim, hidden_dim),
        ...
        # 4. Final MLP mapping to feature_dim
        ...
        self.activation = nn.Sigmoid()  # keep output in [0, 1] if enabled

    def forward(self, normal_features, noise, edge_index, batch):
        """
        noise: (num_nodes, noise_dim) random noise per node
        ...
        # GNN propagation with BN and Dropout
...
class Discriminator(nn.Module):
    def __init__(self, in_dim, hidden_dim):
        ...
        # 1. GNN branch for node features
        self.conv1 = GINConv(nn.Sequential(
        ...
        # 2. MLP branch for joint distribution of x
        self.joint_fc1 = nn.Linear(in_dim * 2, hidden_dim)  # process concatenated feature vector
        ...
        self.readout = global_mean_pool  # can switch to max pool if needed

        # 4. Final decision on the whole graph
    ...
    def forward(self, normal_features, extreme_features, edge_index, batch):
        ...
        out = torch.sigmoid(out)  # probability
        return out
